...
def test_remote_websocket_blocks_non_manual_internal_state_without_submit() -> None:
    reader = FakeRemoteReader()
    settings = remote_settings()
    state = BridgeState(settings)
    state.control_mode = "AUTONOMOUS"  # simulasi state internal abnormal
    app = create_app(settings=settings, state=state, telemetry_reader=reader)
